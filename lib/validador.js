...
// Uma matrícula com zero(s) à esquerda (ex.: "0123456") guardada como célula NUMÉRICA no .ods
// perde o zero na leitura (vira "123456", 6 dígitos), enquanto o PDF da diária preserva o "0"
// literal do texto impresso (7 dígitos). Comparar como string simples faz a mesma pessoa parecer
// "não encontrada" mesmo estando corretamente cadastrada. Preenchendo com zeros à esquerda até um
// tamanho fixo (8, o maior aceito tanto na mensal quanto na diária) os dois lados voltam a bater,
// sem risco de colisão: só o zero à esquerda é perdido nesse tipo de célula, nunca outro dígito.
function norm(m) { return (m || '').replace(/\D/g, '').padStart(8, '0'); }
...
async function validar({ odsInput, sheetName, dia, mes, pdfInput, tipo }) {
  ...
  // Se o PDF não rendeu nenhum militar, avisa de forma clara — senão o resultado fica confuso
  // (todos os que deveriam servir aparecem como "faltando" e a diferença fica muito negativa).
  const diariaVazia = diaria.length === 0;
  if (diariaVazia) {
    avisos.unshift('Nenhum militar foi lido do PDF da diária. Verifique se o arquivo enviado é o "Relatório de acompanhamento diário" do SISP e se o PDF tem texto selecionável (não pode ser uma imagem/escaneado).');
  }
  ...
  for (const [mat, entries] of diariaByMat) {
    ...
    if (m && m.status !== 'SERVICO') {
      escaladoMasNaoDeveria.push({
        // exibe a matrícula original (não a chave normalizada com zeros de padding)
        matricula: entries[0].matricula, nome: entries[0].nome, posto: entries[0].posto,
  ...
  for (const entries of diariaByMat.values()) {
    if (entries.length > 1) {
      const escalasDistintas = [...new Set(entries.map((e) => e.escala.split(' (')[0]))];
      if (escalasDistintas.length > 1) {
        duplicidadeSuspeita.push({ matricula: entries[0].matricula, nome: entries[0].nome, entries });
      }
    }
  }
