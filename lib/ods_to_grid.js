...
function parseSheet(xml, maxCols = 80, maxRows = 300) {
  ...
  while ((rowMatch = rowRegex.exec(xml)) !== null) {
    ...
    while ((cellMatch = cellRegex.exec(rowBody)) !== null) {
      ...
      // Anotações/comentários de célula (office:annotation) trazem seu próprio <text:p>,
      // que a regex de texto abaixo capturaria junto do valor real da célula, corrompendo-o.
      const inner = (cellMatch[4] || '').replace(/<office:annotation\b[\s\S]*?<\/office:annotation>/g, '');
      ...
      if (tag === 'table-cell') {
        ...
        if (valueType === 'float' || valueType === 'currency' || valueType === 'percentage') {
          ...
          // IMPORTANTE: no ODS os espaços costumam vir como elementos <text:s/> (um espaço) ou
          // <text:s text:c="N"/> (N espaços), e o texto de uma célula pode ser quebrado em vários
          // <text:span>. Se apenas removêssemos as tags, "Andre<text:s/>Neves<text:s/>Silva"
          // viraria "AndreNevesSilva". Por isso convertemos <text:s/>, <text:tab/> e
          // <text:line-break/> em espaço ANTES de remover as demais tags, e colapsamos o espaço
          // em branco interno (inclusive quebras de linha de formatação do XML) num único espaço.
          const textMatches = [...inner.matchAll(/<text:p[^>]*>([\s\S]*?)<\/text:p>/g)];
          value = textMatches.map(m => m[1]
            .replace(/<text:s\b[^>]*text:c="(\d+)"[^>]*\/>/g, (_, n) => ' '.repeat(parseInt(n, 10)))
            .replace(/<text:s\b[^>]*\/>/g, ' ')
